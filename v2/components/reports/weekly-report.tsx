...
function exportExcel(data: WeeklyReportData) {
  ...
  const acceptedHeaders = ["When", "What was accepted"];
  const acceptedRows = data.acceptedChanges.map((c) => [c.when, c.summary]);
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([acceptedHeaders, ...acceptedRows]), "Accepted Changes");

  XLSX.writeFile(wb, `PharmaPM_WeeklyReport_${slugFile(data.project.code ?? data.project.name)}.xlsx`);
}
...
export function WeeklyReport() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 print:hidden" data-tour-id="reports-actions">
        <button
      ...
      >
        ...
        {/* O9.3 — integrity caveat travels with the report so the verdict isn't
            read at face value when reported progress may be overstated. */}
        {data.integrityCaveat && (
          <div className="flex items-start gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900 print:break-inside-avoid dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
            <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
            <div>
              <p className="font-semibold">Read the confidence score with caution — reported progress may be overstated.</p>
              <p className="mt-0.5">{data.integrityCaveat}</p>
            </div>
          </div>
        )}
        ...
        {/* O9.2 — what was decided/accepted THIS period (audit log), not just
            the current state. A report without this hides governance history. */}
        <Section title="Decisions Accepted This Period">
          {data.acceptedChanges.length === 0 ? (
            <p className="text-xs text-muted-foreground italic">
              No slips, scope changes, or over-charges were accepted in this reporting cycle.
            </p>
          ) : (
            <div className="rounded-md border border-border overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-muted/40 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                    <th className="px-3 py-1.5 text-left w-28">When</th>
                    <th className="px-3 py-1.5 text-left">What was accepted</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {data.acceptedChanges.map((c) => (
                    <tr key={c.id}>
                      <td className="px-3 py-2 text-muted-foreground whitespace-nowrap">{c.when}</td>
                      <td className="px-3 py-2 text-foreground">{c.summary}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* O8.4 — re-baseline history: when the committed go-live moved, who moved
            it and why. A blank section would be noise, so only render when present. */}
        {data.rebaselines.length > 0 && (
          <Section title="Baseline Changes (Re-baselines)">
            <div className="rounded-md border border-border overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-muted/40 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                    <th className="px-3 py-1.5 text-left">Committed go-live moved</th>
                    <th className="px-3 py-1.5 text-left w-40">When · who</th>
                    <th className="px-3 py-1.5 text-left">Why</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {data.rebaselines.map((r) => (
                    <tr key={r.at}>
                      <td className="px-3 py-2 text-foreground whitespace-nowrap tabular-nums">{fmtReportDate(r.from)} → {fmtReportDate(r.to)}</td>
                      <td className="px-3 py-2 text-muted-foreground whitespace-nowrap">{fmtReportDate(r.at.slice(0, 10))} · {r.by}</td>
                      <td className="px-3 py-2 text-foreground">{r.reason}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="border-t border-border pt-3 flex items-center justify-between text-[10px] text-muted-foreground print:pt-2">
          <span>Generated by PharmaPM · AivelloStudio · {data.reportWeek}</span>
